...
const datepickerDropdowns = $('.date-dropdown');
initCalendar(datepickerDropdowns);
...
function initCalendar(datepickerArea) {
  ...
  const addFooterButtons = (dp, animationCompleted) => {
  ...
  const onShow = (dp, animationCompleted) => {
    console.log('show');
    addFooterButtons(dp, animationCompleted);
    readInputs(dp);
  }

  const onHide = (dp, animationCompleted) => {
    console.log('hide');
  }
